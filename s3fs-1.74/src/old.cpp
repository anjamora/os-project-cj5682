...
ssize_t FdEntity::Read(char* bytes, off_t start, size_t size, bool force_load)
{
  ...
  //myCrypto::decryptFile(fd);
    ...
    if(-1 == (rsize = pread(fd, bytes, contentsIv.length(), start))){
...
ssize_t FdEntity::Write(const char* bytes, off_t start, size_t size)
{
  ...
  //MYCODE
  //string contentsIv = "This is written by Write";
  //const char *bufContents = contentsIv.c_str();
  //ftruncate(fd,size);
  //lseek(fd, 0, SEEK_SET);
  //write(fd, bufContents, contentsIv.length());
  //ENDMYCODE
    ...
    string contentsIv = myCrypto::encryptFile(fd, bytes, start, size);
    //const char *bufContents = contentsIv.c_str();

    //if(-1 == (wsize = pwrite(fd, bufContents, contentsIv.length(), start))){
...
string myCrypto::encryptFile(int fd, const char* bytes, off_t start, size_t size){
  AutoSeededRandomPool prng;

  string ivString, keyString, myKey, contents, enccontents;
  ...
  const byte* byteKey = (const byte*) keyString.data();

  //const byte* byteKey = (const byte*) myKey.data();

  byte byteIv[AES::BLOCKSIZE];
  prng.GenerateBlock(byteIv, sizeof(byteIv));

  CBC_Mode< AES >::Encryption e;
  e.SetKeyWithIV(byteKey, sizeof(byteKey)*2, byteIv);
  ...
  StringSource file(contents, true, 
    new StreamTransformationFilter(e,
      new HexEncoder(
          new StringSink(enccontents)
      )
    )
  );

  StringSource(byteIv, sizeof(byteIv), true,
      new HexEncoder(
          new StringSink(ivString)
      ) // HexEncoder
  ); // StringSource

  string contentsIv = ivString + ":" + enccontents;   //USE THIS CODE**
  //string contentsIv = "This is written by encryptFile";
  //string contentsIv = ivString;
...
string myCrypto::readEncFile(int fd){
  int length;
  length = lseek(fd, 0, SEEK_END);
  char *contents = new char[length];
  lseek(fd,0,SEEK_SET);
  ...
  return scontents.substr(scontents.find(":")+1);
}
...
string myCrypto::getKey(){
  ...
  ifstream myfile;
  myfile.open(fileDir.c_str());
  getline(myfile, key);
  myfile.close();
  return key;
}

string myCrypto::getIv(int fd){
  int length;
  length = lseek(fd, 0, SEEK_END);
  char *contents = new char[length];
  lseek(fd,0,SEEK_SET);
  ...
  return scontents.substr(0, scontents.find(":"));
}

int myCrypto::decryptFile(int fd){

  string myKey, myIv, decodedKey, decodedIv, contents, enccontents;
  ...
  const byte* byteIv = (const byte*) decodedIv.data();
  //const byte* byteKey = (const byte*) decodedKey.data();
  const byte* byteKey = (const byte*) myKey.data();


  CBC_Mode< AES >::Decryption d;
  d.SetKeyWithIV(byteKey, sizeof(byteKey)*2, byteIv);
  

  enccontents = readEncFile(fd);

  StringSource file(enccontents, true,
    new StreamTransformationFilter(d,
      //new HexDecoder(
        new StringSink(contents)
      //)
    )
  );

  //contents = "This was written by Decryption";

  const char *bufContents = contents.c_str();


  lseek(fd, 0, SEEK_SET);
  write(fd, bufContents, contents.length());
  
  return 0;
}

string recovered = "this is a test";

  cout<<"Writing to the file: "<<recovered<<endl<<endl;
  //write recovered text to file
  int inputFile = fd;
  lseek(inputFile,0,0);
  int ss = write(inputFile, recovered.c_str(), recovered.length());
  if(ss == -1){
    cout << "ERROR";
  }
  int i = ftruncate(inputFile, recovered.length());
  ...
  if(i == -1){
    ...
    //Read&Write to file
  ssize_t ret_Iv, ret_Contents, ivOffset;
  ivOffset = (ssize_t)32;
  char bufferContents[size];
  char bufferIv[ivOffset];
  
  //  
  ret_Iv = read(fd, &bufferIv, ivOffset);
  // ret_out = write(fd, &bufferIv, (ssize_t) ret_Iv);
  // size = (ssize_t) ret_out;

  ret_Iv = pread(fd, &bufferContents, size, ivOffset);
  ret_out = write(fd, &bufferContents, (ssize_t) ret_Iv);
  size = (ssize_t) ret_out;
  //Read&Write end